...
(define-key leader-key-map (kbd "ff") 'lsp-format-buffer)
...
;; lsp-ui for additional UI features (optional)
(use-package lsp-ui
  :ensure t
  :commands lsp-ui-mode)

;; Nix LSP support using lsp-nix
(use-package lsp-nix
  :ensure t
  :after lsp-mode
  :custom
...
;; Python configuration using lsp-mode (if you’re using e.g., pylsp)
(use-package python-mode
  :ensure t
  :hook (python-mode . lsp))

;; Custom hook for rustic mode.
;; Ensures that C-c C-c C-r works without asking for confirmation and saves
;; rust buffers without prompting when appropriate.
(defun rk/rustic-mode-hook ()
  (when buffer-file-name
    (setq-local buffer-save-without-query t))
  (add-hook 'before-save-hook 'lsp-format-buffer nil t))

;; Then, later in your config, when setting up rustic:
(use-package rustic
  :ensure t
...
;; Web development configuration remains the same
