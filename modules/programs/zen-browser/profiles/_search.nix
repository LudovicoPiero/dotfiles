...
let
  ...
  processCustomEngineInput =
    ...
    // {
      ...
      loadPath = "[mine.programs.firefox]/${
        lib.showAttrPath (
          modulePath
          ++ [
            "engines"
  ...
  options = {
    ...
    engines = mkOption {
      ...
      description = ''
        ...
        Note, {var}`icon` is also a special option added by `mine.programs.firefox` to make it convenient to specify absolute icon paths.
      '';
    };
