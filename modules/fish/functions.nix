...
let
  ...
  functions = {
    ...
    record = ''
      set -l file ${config.mine.vars.homeDirectory}/Videos/Record/(date +'%F_%H:%M:%S').mp4
    ...
    yt = ''
      set -l sub $argv[1]
      set -e argv[1]
      set -l outdir_audio ${config.mine.vars.homeDirectory}/Media/Audios
      set -l outdir_video ${config.mine.vars.homeDirectory}/Media/Videos
