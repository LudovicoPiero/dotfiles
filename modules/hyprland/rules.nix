...
mkIf cfgmine.hyprland.enable {
  hj.xdg.config.files."hypr/hyprland.conf".text = mkAfter ''
    ...
    windowrule = workspace 2, class:^(firefox|floorp|zen-beta)$
    windowrule = noblur, class:^(firefox|floorp|zen-beta)$
    windowrule = noshadow, class:^(firefox|floorp|zen-beta)$
    windowrule = idleinhibit focus, class:^(firefox|floorp|zen-beta)$, title:^(.*YouTube.*)$
    windowrule = idleinhibit fullscreen, class:^(firefox|floorp|zen-beta)$
